...
const changeSliderValue = value => {
	...
	useValue.textContent = slider.value;
	value == 1
		? translateFunction(averageWear, "use per", userLanguage)
		: translateFunction(averageWear, "uses per", userLanguage);
...
timeFrameSelect.onchange = () => {
	...
	infoText.textContent = `The default slider value is estimated using data from surveyed shoppers. Based on this data, a ${pageItem} is worn ${convertUses(
		slider.timeframe,
		timeFrameSelect.value,
		weeklyItemUse
	).toFixed(2)} times per ${timeFrameSelect.value}.`;
...
const changeLifetimeSliderValue = value => {
	...
	value == 1
		? translateFunction(lifetimeYears, "year", userLanguage)
		: translateFunction(lifetimeYears, "years", userLanguage);
};
